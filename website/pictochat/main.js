...
// Events
const joinRoomEv = new CustomEvent("joinRoom");
const leaveRoomEv = new CustomEvent("leaveRoom");
...
loaderFunc = (loader, resources) => {
    ...
    let dbUrl = "https://pictodb.titangstudio.fr";
    let dbHeaders = {
        "x-api-token" : "zqNZ5WpNa+sw9mUcYPvx0VdP0rf+AtdQ"}
    ...
    window.addEventListener("joinRoom", joinRoom);
    window.addEventListener("leaveRoom", leaveRoom);
    ...
    function generateStageButtons() {
        console.log("Generating stage buttons");
    ...
    function joinRoom() {
        generateRoomButtons({
            count: [0, 0, 0, 0],
            ids: ["room_a", "room_b", "room_c", "room_d"]
        });
        scaleStage();
        joinedRoom = true;
        gotFirstMsg = 0;
    ...
    function enterRoom() {
        ...
        joinMsg.textbox = new PIXI.BitmapText(playerData.name, ndsFont_jl);
        ...
        console.log("Player " + playerData.name + " joined " + roomData.id + ".");
        inRoom = true;
    }

    function convertMessageFromServer(message) {
        // Messages from server are sended with this format
        // - id (primary key, auto-increment)
        // - timestamp (integer)
        // - sender_name (text)
        // - textboxes (serialized JSON)
        // - drawings (serialized JSON)
        // We need to convert it to the format used in the client
        console.log("Converting message from server:", message);
        ...
        return {
            ...
            player: {
                ...
                color: 0 // We don't have color information in the server message, so we set it to 0 (black) by default
            },
            textboxes: Array.isArray(messageObj.textboxes) ? messageObj.textboxes : [],
            drawing: Array.isArray(messageObj.drawings) ? messageObj.drawings : [],
            lines: isNaN(messageObj.lines) ? 1 : Math.max(1, Math.min(5, messageObj.lines))
        };
    }

    async function fetchMessages() {
        const response = await fetch(dbUrl + "/messages",
            {
                method: "GET",
                headers: {
                    ...dbHeaders,
        ...
        if (!response.ok) {
            console.error("Failed to fetch messages:", data.message || response.statusText);
            return;
        }
        var convertedMessages = data.map(convertMessageFromServer);
    ...
    function sendMessage() {
        ...
        if (isMessageValid(message)) {
            ...
            // POST /sendMessage with fetch instead of WebSocket to avoid issues with large messages
            ...
            fetch(dbUrl + "/messages", {
                ...
                body: JSON.stringify({
                    ...
                    textboxes: message.textboxes,
                    drawings: message.drawing,
                    lines: message.lines
                })
            }).then(response => {
                if (!response.ok) {
